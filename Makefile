...
docker-run-fg: docker-stop
	$(DOCKERCMD) run -it --network host $(CAPS) $(VOL_FLAG) $(K8S_ENV_ARGS) --rm $(OPV)

docker-run-bg: docker-stop
	$(DOCKERCMD) run -d --network host $(CAPS) $(VOL_FLAG) $(K8S_ENV_ARGS) --rm --name $(PROJECT) $(OPV)
	$(DOCKERCMD) ps

## attach to console of container running in background
# NOT POSSIBLE with distroless
#docker-cli-bg:
#	$(DOCKERCMD) exec -it $(PROJECT) $(K8S_ENV_ARGS) $(OPV)
